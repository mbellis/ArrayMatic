...
%INPUT PARAMETERS
%       GrpType: (used if RedundantFlag=0) 
%                'split'
%                'tf'
...
global K P
%trs_biolgroups('tf',0,{'WRKY2','WOX8'},{'At5g56270','At5g45980'},{'WRKY2','WOX8'},{[3108],[4022]})
    ...
    switch GrpType
        case 'split'
            ...
            for GrpL=1:GrpNb
                ...
                P.biol.grp.grpBiolRanks{GrpPos,1}=sort(BiolRanks{GrpL});
                GrpPos=length(P.biol.grp.name)+1;
            end
            cd(P.dir.project)
            ...
            for NetL=1:NET_NB
                Pos1=NetGrp(NetL,1);
                Pos2=NetGrp(NetL,2);
                BiolRanks1=P.biol.grp.grpBiolRanks{Pos1};
                BiolRanks2=P.biol.grp.grpBiolRanks{Pos2};
        ...
        case 'tf'
            ...
            if isfield(P.biol,'scouplenb')
                ...
                for BiolL=1:BiolNb
                    ...
                    CurrData=load_data('DataRanks.float32le','./',PsNb,P.point.nb,'single','ieee-le',1:PsNb,CurrPoints);
                    DataRanks(:,BiolL)=mean(CurrData,2);
                end
            end
            ...
            RankLimit=cell(PsNb,1);
            %limit for percentiles
            Limit=[[20,80];[25,75];[30,70]];
            for PsL=1:PsNb
                Ranks=DataRanks(TfPs(PsL),:);
                RankLimit{PsL}=zeros(3,2);
                for LimitL=1:3
                    RankLimit{PsL}(LimitL,1)=length(find(Ranks<=Limit(LimitL,1)));
                    RankLimit{PsL}(LimitL,2)=length(find(Ranks<=Limit(LimitL,2)));
                end
            end
            
            Colors=colors(colormap,PsNb);
            BiolNb=length(BiolPos);
            h=figure;
            set(gcf,'color',[1,1,1])
            ...
            for PsL=1:PsNb
                Ranks=DataRanks(TfPs(PsL),:);
                plot(1:BiolNb,sort(Ranks),'color',Colors(PsL,:))
            end
            set(gca,'box','on')
...
%             UsedPs=[];
%             for PsL=1:PsNb
%                 Pos1=find(RankLimit{PsL}(:,1)>=GrpSize);
%                 if ~isempty(Pos1)
%                     Pos1=Pos1(1);
%                     Pos2=find(RankLimit{PsL}(:,2)>=GrpSize);
%                     if ~isempty(Pos2)
%                         Pos2=Pos2(1);
%                         NetNb=NetNb+1;
%                         UsedPs(end+1,1)=PsL;
...
%                         P.biol.nr{1}.name{NrPos}=sprintf('%s DOWN ps%u %sperc',TfGeneSymbol{PsL},TfPs(PsL),PercName{Pos1});
%                         P.biol.nr{1}.scoupleindex{NrPos}=BiolRanks(find(Ranks<=RankLimit{PsL}(Pos1,1)));
%                         P.biol.nr{1}.type{NrPos}='up/down';
%                         P.biol.nr{1}.corrlimit{NrPos}=P.biol.nr{1}.corrlimit{4};                        
%                         NrPos=length(P.biol.nr{1})+1;
%                         P.biol.nr{1}.startbiol{NrPos}=P.biol.nr{1}.startbiol{4};
%                         P.biol.nr{1}.name{NrPos}=sprintf('%s UP ps%u %sperc',TfGeneSymbol{PsL},TfPs(PsL),PercName{Pos1});
%                         P.biol.nr{1}.scoupleindex{NrPos}=BiolRanks(find(Ranks>=RankLimit{PsL}(Pos1,2)));
%                         P.biol.nr{1}.type{NrPos}='up/down';
%                         P.biol.nr{1}.corrlimit{NrPos}=P.biol.nr{1}.corrlimit{4};                        
%                     end
%                 end                
%             end
            ...
            for PsL=1:PsNb
                ...
                P.biol.nr{1}.startbiol{NrPos}=P.biol.nr{1}.startbiol{4};
                P.biol.nr{1}.name{NrPos}=sprintf('%s DOWN ps%u',TfGeneSymbol{PsL},TfPs(PsL));
                P.biol.nr{1}.scoupleindex{NrPos}=sort(BiolRanks(SortIndex(1:GrpSize)));
                P.biol.nr{1}.type{NrPos}='up/down';
                P.biol.nr{1}.corrlimit{NrPos}=P.biol.nr{1}.corrlimit{4};
                NrPos=length(P.biol.nr{1}.name)+1;
                P.biol.nr{1}.startbiol{NrPos}=P.biol.nr{1}.startbiol{4};
                P.biol.nr{1}.name{NrPos}=sprintf('%s UP ps%u',TfGeneSymbol{PsL},TfPs(PsL));
            ...
            for NetL=1:NetNb
                ...
                K.net{ModelRank}.compNb(NetPos,1)=length(K.net{ModelRank}.biolRank{NetPos,1}{1})*length(K.net{ModelRank}.biolRank{NetPos,1}{2});
                K.net{ModelRank}.fdr(NetPos,:)=[0.01 0.10];
                K.net{ModelRank}.s(NetPos,:)=[0,1];
                K.net{ModelRank}.blocNb(NetPos,1)=ceil(PsNb/BLOC_SIZE);
                K.net{ModelRank}.blocSize(NetPos,1)=BLOC_SIZE;
                K.net{ModelRank}.comment{NetPos,1}='';
